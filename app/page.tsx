...
const interval = setInterval(() => {
  setLoadingStep((s) => (s < loadingMessages.length - 1 ? s + 1 : s));
    ...
    try {
      ...
      const data = (await res.json()) as { result?: AuditResult; error?: string };
      if (!res.ok) throw new Error(data?.error || "Analyse fehlgeschlagen.");
      if (!data.result) throw new Error("Kein Ergebnis erhalten.");
  ...
  return (
    <main style={{ maxWidth: 980, margin: "32px auto", padding: 18, fontFamily: "system-ui", background: "f6f8fc", borderRadius:24, }}>
  ...
  <h1 style={{ fontSize: 34, margin: 0,color: "#0f172a",
   letterSpacing: -0.6 }}>
    AI Website Reality Check
  </h1>

  <p style={{ color: "#475569", marginTop: 10, lineHeight: 1.6, marginBottom: 0 }}>
    <strong>Schnelle, ehrliche Analyse</strong> deiner Website aus Conversion-
    & Messaging-Sicht.<br />
    Kein SEO-Tool. Kein Design-Check. Fokus auf{" "}
    <strong>Klarheit, Vertrauen und Handlungsstärke</strong>.
  </p>
</div>


      {/* Claim / Erwartungsmanagement */}
...
>
            ...
            {/* Score Erklärung mit Markierung */}
            {(() => {
          ...
          <div style={{ 
            ...
            <EmailGateInline websiteUrl={url} />
